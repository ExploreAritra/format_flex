...
class FfmpegService {
  ...
  String buildCommand({
    ...
    final vArgs = <String>['-c:v', o.vcodec.ffmpegName];
    switch (o.vcodec) {
      case VCodec.h264:
        vArgs.addAll(['-preset', 'medium', '-profile:v', 'high', '-pix_fmt', 'yuv420p']);
        if (o.useCrf) vArgs.addAll(['-crf', '${o.crf}']); else vArgs.addAll(['-b:v', '${o.vBitrateK}k']);
        break;
      case VCodec.hevc:
        vArgs.addAll(['-preset', 'medium', '-pix_fmt', 'yuv420p', '-tag:v', 'hvc1']);
        if (o.useCrf) vArgs.addAll(['-crf', '${o.crf}']); else vArgs.addAll(['-b:v', '${o.vBitrateK}k']);
        break;
      case VCodec.vp9:
        ...
        break;
      case VCodec.av1:
        vArgs.addAll(['-pix_fmt', 'yuv420p', '-cpu-used', '4']);
        if (o.useCrf) vArgs.addAll(['-crf', '${o.crf}', '-b:v', '0']); else vArgs.addAll(['-b:v', '${o.vBitrateK}k']);
        break;
    }
...
class _ConverterHomeState extends State<ConverterHome> {
  ...
  Future<void> _convert() async {
    ...
    setState(() { _status = 'Probing…'; _progress = 0; _durationMs = null; });
    ...
    await _svc.convert(
      ...
      onDone: (msg) => setState(() { _status = 'Done: ${opts.computedOutputPath()}'; _progress = 1.0; }),
      onError: (msg) => setState(() { _status = msg; }),
      onSession: (id) => _sessionId = id,
    );
  }

  Future<void> _cancel() async { if (_sessionId != null) await FFmpegKit.cancel(_sessionId!); }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(onPressed: _cancel, icon: const Icon(Icons.close))],
      ),
      body: ListView(
        ...
        children: [
          ...
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: _pickInput, icon: const Icon(Icons.video_file), label: const Text('Pick Video'))),
          ]),
          const SizedBox(height: 16),
          ...
          Row(children: [
            ...
            FilledButton.tonal(onPressed: _chooseOutputFolder, child: const Text('Choose…')),
          ]),
          const SizedBox(height: 12),
          TextField(
            onChanged: (v) => setState(() { opts.outputFileName = v; opts.output = opts.computedOutputPath(); }),
          ...
          _Labeled(
            ...
            child: DropdownButtonFormField<Preset>(
              ...
              onChanged: (pz) { if (pz == null) return; setState(() { opts.applyPreset(pz); _suggestOutputName(opts.input.isNotEmpty ? opts.input : opts.outputFileName); }); },
            ),
          ),
          const SizedBox(height: 12),
          // Container
          _Labeled(
            ...
            child: DropdownButtonFormField<ContainerFmt>(
              ...
              onChanged: (v) { if (v==null) return; setState(() { opts.container = v; if (opts.input.isNotEmpty) _suggestOutputName(opts.input); }); },
            ),
          ),
          const SizedBox(height: 12),
          // Video codec
          _Labeled(
            ...
            child: DropdownButtonFormField<VCodec>(
              ...
              onChanged: (v) { if (v==null) return; setState(() { opts.vcodec = v; }); },
            ),
          ),
          const SizedBox(height: 12),
          // Audio codec
          _Labeled(
            ...
            child: DropdownButtonFormField<ACodec>(
              ...
              onChanged: (v) { if (v==null) return; setState(() { opts.acodec = v; }); },
            ),
          ),
          const SizedBox(height: 12),
          // Resolution
          _Labeled(
            ...
            child: DropdownButtonFormField<Resolution>(
              ...
              onChanged: (r) { if (r==null) return; setState(() { opts.resolution = r; if (opts.input.isNotEmpty) _suggestOutputName(opts.input); }); },
            ),
          ),
          const SizedBox(height: 12),
          // FPS
          _Labeled(
            ...
            child: DropdownButtonFormField<double?>(
              ...
              items: [null, 24.0, 25.0, 29.97, 30.0, 50.0, 59.94, 60.0]
                  ...
                  .toList(),
              onChanged: (f) => setState(() { opts.fps = f; }),
            ),
          ),
          const SizedBox(height: 12),
          // Quality vs Bitrate
          SwitchListTile(
            ...
            value: opts.useCrf,
            onChanged: (v) => setState(() { opts.useCrf = v; }),
          ),
          if (opts.useCrf)
            ...
            _NumberField(label: 'Video Bitrate (kbps)', value: opts.vBitrateK.toDouble(), min: 250, max: 20000, step: 250, onChanged: (v) => setState(() { opts.vBitrateK = v.round(); })),
          ...
          _Labeled(
            ...
            child: DropdownButtonFormField<int>(
              ...
              onChanged: (c) => setState(() { if (c!=null) opts.audioChannels = c; }),
            ),
          ),
          const SizedBox(height: 12),
          _Labeled(
            ...
            child: DropdownButtonFormField<int>(
              ...
              onChanged: (sr) => setState(() { if (sr!=null) opts.sampleRate = sr; }),
            ),
          ),
          const SizedBox(height: 12),
          CheckboxListTile(
            value: opts.toneMapHdrToSdr,
            onChanged: (v) => setState(() { opts.toneMapHdrToSdr = v ?? true; }),
            title: const Text('Tone-map HDR → SDR when needed'),
            subtitle: const Text('Improves compatibility on older projectors/TVs'),
          ),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: canConvert ? _convert : null, icon: const Icon(Icons.play_arrow), label: const Text('Convert'))),
          ]),
          const SizedBox(height: 24),
          const _TipsBox(),
        ],
...
class _NumberField extends StatefulWidget {
  final String label; final double value; final double min; final double max; final double step; final ValueChanged<double> onChanged;
...
class _NumberFieldState extends State<_NumberField> {
  ...
  @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(widget.label),
    Row(children: [
      Expanded(child: Slider(value: _v.clamp(widget.min, widget.max), min: widget.min, max: widget.max, divisions: ((widget.max-widget.min)/widget.step).round(), label: _v.round().toString(), onChanged: (v){ setState(()=>_v=v); widget.onChanged(v); })),
